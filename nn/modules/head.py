...
#             # 1. 计算显著性 (Saliency)
#             # [修正] 移除 torch.sigmoid！直接输出 Logits。
#             # 理由：
#             # 1. CascadeLoss 使用 BCEWithLogitsLoss，需要 Logits。
#             # 2. STN 的 Softmax 需要 Logits 才能产生尖锐的概率分布。
#             saliency_logits = self.cv2[i](feature) 
            
#             # 2. 计算不确定性 (Uncertainty)
#             # Softplus 是可以保留的，因为我们需要它恒为正
#             uncertainty = self.cv3[i](x[i])
#             uncertainty = F.softplus(uncertainty)

#             # 3. 拼接输出 -> Channel 0: Logits, Channel 1: Uncertainty
#             # 注意：这意味着后续使用 Channel 0 时，如果需要概率图用于可视化，必须手动加 Sigmoid
#             x[i] = torch.cat((saliency_logits, uncertainty), 1)
...
class CoarseDetect(nn.Module):
    ...
    Coarse Detection Head for Cascade Focusing.
    Structure:
        - Branch 1: Saliency Map (Output 1 channel, Logits for BCEWithLogitsLoss)
        - Branch 2: Uncertainty Map (Output 1 channel, Softplus activation)
    Output:
        [B, 2, H, W] per scale level.
    """
    ...
    def __init__(self, nc=1, ch=()): 
        ...
        # 内部通道数设定
        c2, c3 = max((16, ch[0] // 4, 64)), max(ch[0], 64) 
        
        # 定义两个卷积分支
        # cv2: 负责 Saliency (注意最后一层是 Conv2d 无激活)
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, 1, 1)) for x in ch
        )
        # cv3: 负责 Uncertainty
        ...
        # [新增] 执行零初始化，解决冷启动 STN 乱飘的问题
        self.initialize_biases()

    def initialize_biases(self):
        """
        强制初始化 Saliency 分支的最后一层，使其输出均值为 0 的 Logits。
        效果：初始概率 ~0.5 (Uniform)，保证 STN 初始时刻聚焦于图像中心。
        """
        for sequential_block in self.cv2:
            # 取出最后一个 1x1 卷积层 (nn.Conv2d)
            final_conv = sequential_block[-1]
            
            # 权重设为极小的高斯噪声 (接近0但不完全为0，利于梯度流动)
    ...
    def forward(self, x):
        ...
        Args:
            x (list): Features from FPN/Neck.
        Returns:
            list[Tensor]: Each tensor shape [B, 2, H, W] (Saliency Logits, Uncertainty)
        """
        for i in range(self.nl):
            # Input x[i] shape: [B, C_in, H, W]
            feature = x[i]
            ...
            # 1. 计算显著性 (Saliency)
            # 输出 Logits (范围 -inf ~ +inf)，不做 Sigmoid
            saliency_logits = self.cv2[i](feature) 
            
            # 2. 计算不确定性 (Uncertainty)
            # 使用 Softplus 保证值为正 (0 ~ +inf)
            uncertainty_raw = self.cv3[i](feature) # 注意这里输入应该是 feature 而不是 x[i] (虽然x[i]没变，但用feature变量更清晰)
